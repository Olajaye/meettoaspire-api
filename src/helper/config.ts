...
const Config = {
  ...
  app: {
    ...
    port: () => Utils.env('PORT') ?? 3004,
    name: () => Utils.env('APP_NAME', 'Eyrie & Obra Property'),
    description: Utils.env(
      'APP_DESCRIPTION',
      'An all-in-one online platform designed to meet the needs of individuals and professionals in the real estate industry',
    ),
    defaultAdminEmail: Utils.env(
      'DEFAULT_ADMIN_USER',
      'superadmin@example.com',
    ),
    defaultEyrienobraAgent: () => Utils.env('DEFAULT_EYRIENOBRA_AGENT'),
